...
def predict_git_category(sample_message, model):
    encoded_message = config.TOKENIZER.encode_plus(sample_message, max_length=config.MAX_LEN, truncation=True, add_special_tokens=True,
                                                   return_token_type_ids=False, pad_to_max_length=True, return_attention_mask=True, return_tensors='pt')
...
while True:
    ...
    if sample_message == 'exit':
        break
    print("This is a " + predict_git_category(sample_message, bug_predictor_model))
